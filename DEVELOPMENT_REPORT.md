...
### Build/verification
`tsc --noEmit` clean, `ng build` clean (one non-blocking CSS budget warning on `roleplay-session.scss`, 116 bytes over 4kB — cosmetic, not fixed this round). Dashboard, adaptive Mock Interview, and the Call Center Simulator flow all checked live in Chrome against the real Supabase-backed test account with no console errors.
