...
## Vocabulary Engine — §29

Full rebuild of Vocabulary per the "VOCABULARY ENGINE" brief. Audited first (see below), then built a real selection/activity engine on top of the existing Supabase-backed content instead of a static browse list + one quiz mode.

### Audit findings (before any changes)
- ~104 words total across `vocabulary.sql` (50, A1-B1) + `vocabulary-b2-c1-c2.sql` (42, B2-C2) — heavily B2/C1/C2-light relative to the brief's ask.
- `VocabularyWord` was flat: `term`/`translation`/one `example`/`category`/`level`. No definition, no multiple examples, no synonyms/antonyms/collocations/register — a word was taught as word → translation only, exactly what the brief said not to do.
- Vocabulary page (`vocabulary.ts`) was browse/filter only — no practice activity lived there. **Vocabulary Rush** was the only quiz, and it was one activity type forever (translation multiple-choice), picking words with `Array.sort(() => Math.random() - 0.5)` — pure random, no recency tracking, could show the same word two rounds in a row.
- **Review** (`SpacedRepetitionService`) was the one genuinely solid piece already: a real simplified SM-2 (`nextIntervalDays`/`nextDifficulty`, unit-tested) driving `dueWords()`. It only ever pulled "due" words, though — no new/weak mixing, and no per-word mastery ladder beyond a raw accuracy percent.
- No recent-history tracking anywhere (§8/§31 of the brief) — nothing prevented the same word or question from reappearing immediately.
- No cross-module reuse: vocabulary lived only in Vocabulary/Vocabulary Rush/Review; Roleplay/Reading/Interview content is entirely separate, unconnected data.

### What was built
- **Lexical knowledge model** (§3-§4): `VocabularyWord` extended with `partOfSpeech`, `definition`, `examples[]` (3-4 varied sentences per word, not 1), `synonyms`, `antonyms`, `collocations`, `register` (casual/neutral/formal), `topic`, `relatedWords` (word-family links), `sense` (polysemy label), `commonMistake` — all optional so older/thin rows still render. `VocabularyCategory` gained 12 new topics (Customer Service, Call Center, Finance, Leadership, Career, Communication, Emotions, Environment, Society, Media, Relationships, Shopping, Sales) per §25/§34.
- **Content pool** (`supabase/vocabulary-engine-v2.sql`): additive migration, 124 new rows (~228 total with the existing content) — real coverage gaps filled per the brief's per-level lists (A1 greetings/numbers/colors/clothes/routines; A2 shopping/travel/health/hobbies/phrasal verbs; B1 workplace/emotions/environment/idioms/collocations; B2 customer-service/business/negotiation/leadership/word-formation families; C1 academic/discourse-markers/ethics/social-issues; C2 register-nuance ladder — big/substantial/considerable/enormous/massive/significant, explicitly with register + a `commonMistake` note on each, per the brief's own example), a **polysemy set** (charge/issue/run/hold/point, each sense its own row with its own definition/translation/example — §16), a **phrasal-verb library** (§18), and a **call-center vocabulary set** (§34: first-call resolution, average handle time, QA, CSAT, upsell, chargeback, de-escalate, etc.) feeding the Call Center topic. Every example sentence is workplace/customer-service/travel/tech-grounded, not "the dog eats the apple" (§6). Honest scope note: this is ~228 words, not the 500-5000 the brief floated — see Known Limitations.
- **Selection algorithm** (`VocabularyEngineService` + pure `selectSession()`/`selectActivityType()` functions, §9/§11/§37): status ladder (New → Learning → Practicing → Familiar → Mastered) derived from existing `review_items` accuracy (`statusFromReviewItem()` in `SpacedRepetitionService`, exported+tested); a session mixes ~60% new / 25% due-for-review / 15% weak (learning/practicing status), backfilling from the rest of the pool if a bucket runs dry (small C2 pool) rather than returning short. Recency: last 40 shown word ids tracked in a signal + `localStorage` (`STORAGE_KEYS.recentVocabulary`), excluded from selection when enough fresh words remain, but never blocking a word forever (§8) — falls back to the full bucket, and only as a last resort reaches into recently-shown words during backfill. `Math.random()` only breaks ties inside an already-level/status/recency-filtered set, never drives selection directly (§36).
- **12 activity types** (§12, `VocabularyActivityService`): Multiple Choice, Context Choice, Fill in the Blank, Matching, Synonym/Antonym/Collocation/Phrasal Verb/Register Challenge, Definition, Translation, Reverse Translation. Each word only gets types it has real metadata for (`supportsActivityType()`) — no invented distractors (§38): if a word has no synonyms, Synonym Challenge is simply not offered for it, and `build()` returns `null` rather than fabricate options. Distractors are always pulled from other real words in the pool (same pattern Vocabulary Rush already used, generalized). Activity type per word also avoids the last 4 types just used (`selectActivityType`), so two consecutive words don't repeat the same interaction shape.
- **New "Vocabulary Practice" session** (`/vocabulary-practice`, `VocabularyPracticeComponent`): 10-word mixed sessions (level filter or "All levels"), each word rendered with whatever activity type the engine picked, explanation + common-mistake callout shown after answering, grades every answer through the existing `SpacedRepetitionService.grade()` so it feeds the same mastery/due-date data Review already uses — one shared source of truth, not a second progress system. Linked from the Vocabulary page above the existing Vocabulary Rush CTA. Vocabulary Rush itself is untouched (still a valid, different-feeling activity — timed, translation-only, by design).
- Vocabulary page word cards now show `definition`, `register`, and `collocations` when present (§3/§4 visible in the browse view too, not just practice).

### Repetition algorithm (§9, §10) — deliberately simple, per spec
No full SM-2 rebuild — `SpacedRepetitionService`'s existing simplified SM-2 (`nextIntervalDays`/`nextDifficulty`) was already correct and already tested; reused as-is. The new engine work sits one layer up: *which* due/new/weak words to show and *which activity* to show them with, which the old system never decided at all.

### Mistake tracking
Every practice answer already flows into `review_items` (accuracy → status ladder → the "weak" bucket resurfaces the word sooner). Explicitly **not** wired into the separate text-based `MistakeMemoryService` ("My Mistakes") — that system pattern-matches wrong *text* from open-ended Interview/Roleplay answers, a different shape of data than a wrong vocabulary MC option. Unifying them would need a real design decision (a wrong vocabulary answer isn't a "wrong sentence"), not a mechanical wire-up — left as a Known Limitation rather than forced.

### Cross-module integration (§29-§30)
**Not done this pass**, and said plainly rather than glossed over: wiring learned vocabulary into Reading/Listening/Writing/Speaking/Roleplay/Interview/Games/Exams content generation is a genuinely separate, large effort — six-plus modules each with their own content model, none of which currently take a "vocabulary to reinforce" input. The call-center vocabulary set was authored so the *terms* already overlap with Roleplay/Interview content (escalate, refund, troubleshoot, retention, etc.), which gets partial reinforcement for free by simple word overlap, but there's no engineered "words you just learned get inserted into your next Reading passage" mechanism. Flagged as the highest-value follow-up.

### Testing (§39)
`vocabulary-engine.service.spec.ts` (11 tests): session ratio (60/25/15) under a healthy pool, never over-returning, recency exclusion, recency fallback-when-forced, level containment, activity-type eligibility per metadata field, and activity-type fallback when nothing fresh is eligible. `vocabulary-activity.service.spec.ts` (6 tests): exactly one correct option present, no duplicate options ever (§38 — never two "correct" answers), `null` returned instead of a fabricated question when metadata is missing, fill-blank actually blanks the term, collocation blanks only the trailing word, register challenge never duplicates the correct register as a distractor. All 122 existing + 23 new tests pass; `tsc --noEmit` and `ng build --configuration production` both clean.

### Known Limitations
- **Content volume**: ~228 words, not the 500-5000+ the brief's language gestured at. Every new row was hand-authored to the brief's own quality bar (§33: "no llenar el sistema con palabras rarísimas") rather than mass-generated — the honest trade-off is fewer words, all real.
- **Cross-module integration** (§29-§30, §34's "esto debe alimentar Interview/Roleplay/Career Path") not engineered — see above.
- **Mistake-system unification** not done — vocabulary mistakes live in `review_items`/status ladder, not in the separate `MistakeMemoryService` "My Mistakes" list.
- **No live Chrome verification** this pass — no active authenticated session available; work is structurally verified (typed, tested, built) but not re-confirmed against the running app in a browser.
- `Vocabulary Practice` always draws from every loaded word regardless of the user's current placement level unless a level filter is picked manually — it doesn't yet default the filter to the user's own CEFR level.
...
- **Vocabulary Engine**: lexical-knowledge data model, 124-row content expansion (polysemy, phrasal verbs, call-center vocabulary, register-nuance ladder), status-aware new/review/weak selection with recency tracking, 12-activity-type generator, new Vocabulary Practice session, 17 new tests — §29.
...
- Live Chrome / authenticated-session verification of §25-§29's work (no active session in any of these passes) — this specifically includes §28's own refresh-test/logout-login-test recommendation, which needs a real session to run.
- **Question-pool content volume** (spec §21): B2/C1/C2 grammar/vocabulary topics still have only 2 authored MC exercises each — real variety within a single topic (beyond option shuffling) needs substantially more content, not an engineering fix. Vocabulary specifically improved this pass (§29); Grammar/Lessons/Listening did not.
...
- **Vocabulary cross-module integration** (§29-§30/§34 of the Vocabulary Engine brief): learned words are not yet fed into Reading/Listening/Writing/Speaking/Roleplay/Interview/Games/Exams content generation. See §29's Known Limitations.
- PWA manifest icons are still generic Angular CLI placeholders (pre-existing, unrelated to this task).

## Known Issues
- None newly introduced — every change across §25-§29 went through the full `tsc`/lint/test/build cycle clean, and `ng test` stayed green throughout (122/122 after §29's additions; one pre-existing test updated in §27 to match a deliberate timing change, not a regression).
...
- **`supabase/vocabulary-engine-v2.sql` has not been run yet** — required before any of §29's new columns/content are visible; same pattern as the still-pending `vocabulary-b2-c1-c2.sql`/`activity-log-b2-c1-c2.sql` migrations noted in earlier sections.

## Recommended Next Steps
1. Run `supabase/vocabulary-engine-v2.sql` in the Supabase SQL editor (after the earlier vocabulary migrations), then log in and live-verify Vocabulary Practice: confirm new/review/weak words mix, activity types vary session to session, B2/C1/C2 filters actually show B2/C1/C2-only content.
2. Vocabulary cross-module integration (§29-§30) is the natural next Vocabulary Engine follow-up: start with one high-signal connection (e.g. Roleplay scenario generation preferring recently-learned call-center terms) rather than all six modules at once.
3. Pick mini-games as the next dedicated pass — it's the largest remaining piece of the B2/C1/C2 brief and deserves its own focused implementation rather than being squeezed in.
4. A real question-pool expansion for Grammar/Lessons/Listening (more MC exercises per topic) is the next highest-leverage step for the "repetitive activities" complaint outside Vocabulary, which §29 already improved.
5. A "Practice My Mistakes" micro-remediation flow (§22-§24/§29 of the Feedback brief) is a natural next step for the Feedback Engine, building on the existing `MistakeMemoryService` rather than a new system.
6. A genuine breakpoint-by-breakpoint pass (DevTools device toolbar or a real phone) on everything built across §25-§29, once live.
